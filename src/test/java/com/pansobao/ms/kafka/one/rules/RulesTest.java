package com.pansobao.ms.kafka.one.rules;
 ...
 * packages: Debe ir el nombre del package del ms.
  */
@AnalyzeClasses(packages = "com.pansobao.ms.kafka.one", importOptions = ImportOption.DoNotIncludeTests.class)
public class RulesTest {
    ...
    public static final ArchRule layer_dependencies = Architectures.layeredArchitecture().consideringOnlyDependenciesInLayers()
            ...
            .layer(CONFIG).definedBy("com.pansobao.ms.kafka.one.config..")
            .layer(DOMAIN).definedBy("com.pansobao.ms.kafka.one.domain..")
            .layer(ADAPTER).definedBy("com.pansobao.ms.kafka.one.adapter..")
            .layer(APPLICATION).definedBy("com.pansobao.ms.kafka.one.application..")
